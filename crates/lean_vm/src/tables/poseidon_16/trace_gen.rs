...
pub(super) fn generate_trace_rows_for_perm<F: Algebra<KoalaBear> + Copy>(perm: &mut Poseidon1Cols16<&mut F>) {
    ...
    for (checkpoint, constants) in perm
        .initial_full_rounds
        .iter_mut()
        .zip(initial_constants.iter())
    {
        generate_1_full_round(&mut state, checkpoint, constants);
    }
    ...
    for (checkpoint, constants) in perm
        .final_full_rounds
        .iter_mut()
        .zip(final_constants.iter())
    {
        generate_1_full_round(&mut state, checkpoint, constants);
    }

    let flag_permute = *perm.flag_permute;
    generate_output(&state, &inputs, &mut perm.outputs_left, &mut perm.outputs_right, flag_permute);
}
...
fn generate_1_full_round<F: Algebra<KoalaBear> + Copy>(
    ...
    checkpoint.iter_mut().zip(*state).for_each(|(chk, x)| {
        **chk = x;
    });
}

#[inline]
fn generate_output<F: Algebra<KoalaBear> + Copy>(
    final_state: &[F; WIDTH],
    inputs: &[F; WIDTH],
    ...
    for i in 0..(WIDTH / 2) {
        let compression_value = final_state[i] + inputs[i];
        *outputs_left[i] = (F::ONE - flag_permute) * compression_value + flag_permute * final_state[i];
        *outputs_right[i] = flag_permute * final_state[i + WIDTH / 2];
    }
}
