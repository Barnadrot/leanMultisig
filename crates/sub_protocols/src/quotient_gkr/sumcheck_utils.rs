...
{
    // Manual factoring: each `sumcheck_quadratic((a0,a1),(b0,b1))` computes
    // `(b0*a0, (b1-b0)*(a1-a0))`. Across the three calls, `dl.1-dl.0` and
    // `dr.1-dr.0` each appear twice. Compute all four deltas once.
    ...
    let c2_den = delta_dl * delta_dr;
    let c0_a = dr.0 * nl.0;
    let c2_a = delta_nl * delta_dr;
    let c0_b = dl.0 * nr.0;
    let c2_b = delta_nr * delta_dl;
