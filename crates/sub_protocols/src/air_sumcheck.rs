    ...
    A::ExtraData: AlphaPowers<EF>,
{
    ...
    if let Some(low_degree) = computation.low_degree_air() {
        if let MleGroupRef::BasePacked(cols) = multilinears {
            return compute_raw_poly_split_basepacked(
                cols,
...
#[allow(clippy::too_many_arguments)]
fn compute_raw_poly_split_basepacked<EF, A>(
    cols: &[&[PFPacking<EF>]],
    get_split_eq: impl Fn(usize) -> EFPacking<EF> + Sync + Send,
    computation: &A,
    extra_data: &A::ExtraData,
    fold_bit: usize,
    active_count_pairs: usize,
    ...
    A::ExtraData: AlphaPowers<EF>,
{
    ...
    let eval_points: Vec<PF<EF>> = std::iter::once(PF::<EF>::ZERO)
        .chain((2..=(low_degree + 1) as u64).map(|z| PF::<EF>::from_u64(z)))
        .collect();
    let target_points: Vec<PF<EF>> =
        ((low_degree + 2) as u64..=degree as u64).map(|z| PF::<EF>::from_u64(z)).collect();

    let lagrange_coeffs: Vec<Vec<EFPacking<EF>>> = target_points
        .iter()
        .map(|&tz| {
            eval_points
                .iter()
                ...
                .map(|(i, &zi)| {
                    ...
                    for (j, &zj) in eval_points.iter().enumerate() {
                        if j != i {
                            num *= tz - zj;
                            den *= zi - zj;
                        }
    ...
    let state_interp_coeffs: Vec<PFPacking<EF>> = target_points
        .iter()
    ...
    let acc = (0..active_count_pairs)
        ...
        .fold(
            || {
                (
                    ...
                    vec![EFPacking::<EF>::ZERO; n_full],
                    {
                        ...
                        v.resize(state_cap, PFPacking::<EF>::ZERO);
                        v
                    },
                    {
                        let mut v = Vec::with_capacity(state_cap);
            ...
            |(mut acc, mut point, mut diff, mut low_evals, mut state_0_buf, mut state_2_buf, mut cached_buf),
             new_j| {
                let i_hi = new_j >> fold_bit;
                let i_lo = new_j & lo_mask;
                let i0 = (i_hi << (fold_bit + 1)) | i_lo;
                ...
                }
                {
                    let mut folder = ConstraintFolderPacked {
                        ...
                        skip_low: false,
                        accumulator_low: EFPacking::<EF>::ZERO,
                        cached_state: std::mem::replace(&mut state_2_buf, Vec::new()),
                    };
                    Air::eval(computation, &mut folder, extra_data);
                    acc[1] += folder.accumulator * partial_eq;
                    low_evals[1] = folder.accumulator_low;
                    state_2_buf = folder.cached_state;
                }

                // z=3,...,low_degree+1: full eval (capture low only)
                ...
                // Phase 2: z=low_degree+2,...,degree (skip low, interpolate)
                let state_len = state_0_buf.len().min(state_2_buf.len());
                for t in 0..target_points.len() {
                    for k in 0..n_cols {
                        point[k] += diff[k];
                    }

                    // Interpolate post-partial state
                    let coeff = state_interp_coeffs[t];
                    for i in 0..state_len {
                        cached_buf[i] = state_0_buf[i] + coeff * (state_2_buf[i] - state_0_buf[i]);
                    }
                    ...
                    let mut folder = ConstraintFolderPacked {
                        ...
                        skip_low: true,
                        accumulator_low: EFPacking::<EF>::ZERO,
                        cached_state: std::mem::replace(&mut cached_buf, Vec::new()),
                    };
                    Air::eval(computation, &mut folder, extra_data);
                    cached_buf = folder.cached_state;

                    // Interpolate P_low at target z
                ...
                (acc, point, diff, low_evals, state_0_buf, state_2_buf, cached_buf)
            },
        )
        .map(|(acc, ..)| acc)
        .reduce(
