...
/// Cache-line-pad a static atomic so that writes by the main thread (in
/// `begin_phase`/`end_phase`) don't invalidate cache lines that worker threads
/// are reading on every alloc/dealloc. Apple Silicon cache lines are 128 bytes
/// (vs 64 on x86); pad to that.
#[repr(align(128))]
struct CachePad<T>(T);
...
/// every `dealloc` to test whether a pointer belongs to us. Padded onto its own
/// cache line so begin/end-phase writes to GENERATION/ARENA_ACTIVE don't
/// invalidate the dealloc-read line on worker threads.
