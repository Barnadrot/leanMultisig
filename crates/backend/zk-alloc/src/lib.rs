...
/// path where it sees `ARENA_ACTIVE = false` and routes to System. This lets
/// the alloc fast path drop its `ARENA_ACTIVE` check (the gen-cache mismatch
/// already gates entry to the bump body).
pub fn end_phase() {
...
unsafe impl GlobalAlloc for ZkAllocator {
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        // Small allocs bypass arena. Done before any phase check: phase-crossing
        // safety is the same regardless of whether we are mid-phase, and the
        // System call path is identical for in-phase-small and out-of-phase-small.
        if layout.size() < MIN_ARENA_BYTES {
            return unsafe { std::alloc::System.alloc(layout) };
        }
        // The single hot-path gate: in-phase + slab-claimed iff cached gen
        // matches global. begin_phase()/end_phase() each bump GENERATION, so a
        // mismatch means either no phase is active or this thread hasn't
        // claimed its slab in the current phase.
        let generation = GENERATION.load(Ordering::Relaxed);
