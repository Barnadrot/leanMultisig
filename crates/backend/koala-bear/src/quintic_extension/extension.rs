...
/// Karatsuba-style quintic mul mod (X^5 + X^2 - 1).
///
/// Splits each operand 3+2 (low = degrees 0..2, high = degrees 3..4) and
/// uses Karatsuba for the cross term to compute the 9-coefficient convolution
/// in **21 base muls** vs schoolbook's 25 (al*bl: 9, ah*bh: 3, (al+ah)(bl+bh): 9).
/// Saves 4 muls per quintic-mul call at the cost of ~9 extra adds. On NEON
/// Montgomery (5-cycle mul-latency, 1/cycle throughput; 3-cycle adds, multi-port),
/// the mul-saving dominates the add-cost on the throughput-limited path.
///
/// Reduction: X^5 = 1 - X^2, X^6 = X - X^3, X^7 = X^2 - X^4, X^8 = X^3 + X^2 - 1.
#[inline(always)]
pub fn quintic_mul_karatsuba<T>(a: &[T; 5], b: &[T; 5]) -> [T; 5]
where
    T: Copy
...
{
    // al * bl (3x3 schoolbook, 9 muls, 5 result coefs)
    let p0 = a[0] * b[0];
    let p1 = a[0] * b[1] + a[1] * b[0];
    let p2 = a[0] * b[2] + a[1] * b[1] + a[2] * b[0];
    let p3 = a[1] * b[2] + a[2] * b[1];
    let p4 = a[2] * b[2];
    // ah * bh (2x2 Karatsuba, 3 muls, 3 result coefs)
    let q0 = a[3] * b[3];
    let q2 = a[4] * b[4];
    let cross_h = (a[3] + a[4]) * (b[3] + b[4]);
    let q1 = cross_h - q0 - q2;
    // (al + ah_pad) * (bl + bh_pad) where ah_pad / bh_pad is ah / bh extended with 0
    // at index 2 — so the sum is a 3-coef polynomial. 3x3 schoolbook = 9 muls.
    let s0 = a[0] + a[3];
    let s1 = a[1] + a[4];
    let s2 = a[2];
    let t0 = b[0] + b[3];
    ...
    let m0 = s0 * t0;
    let m1 = s0 * t1 + s1 * t0;
    let m2 = s0 * t2 + s1 * t1 + s2 * t0;
    let m3 = s1 * t2 + s2 * t1;
    let m4 = s2 * t2;
    // Cross convolution = mid - al*bl - ah*bh (zero-padded to 5)
    let r0 = m0 - p0 - q0;
    let r1 = m1 - p1 - q1;
    let r2 = m2 - p2 - q2;
    let r3 = m3 - p3;
    let r4 = m4 - p4;
    // 9-coef product: c[k] for k in 0..9
    //   c[0..5] = al*bl
    //   c[3..8] += cross (X^3-shifted)
    //   c[6..9] += ah*bh (X^6-shifted)
    let c0 = p0;
    let c1 = p1;
    let c2 = p2;
    let c3 = p3 + r0;
