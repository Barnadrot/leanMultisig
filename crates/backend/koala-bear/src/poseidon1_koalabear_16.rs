...
impl Poseidon1KoalaBear16 {
    ...
    /// Interleaved 2x compression: processes two states through the Poseidon permutation
    /// with round-level interleaving to hide NEON pipeline latency.
    #[cfg(all(target_arch = "aarch64", target_feature = "neon"))]
    #[inline(always)]
    pub fn compress_in_place_2x(&self, state_a: &mut [PackedKB; 16], state_b: &mut [PackedKB; 16]) {
        use crate::{InternalLayer16, add_rc_and_sbox, sbox};
        use core::mem::transmute;

        let initial_a = *state_a;
        let initial_b = *state_b;
        ...
        // --- Initial full rounds (first 3 of 4), interleaved ---
        for round_constants in &simd.packed_initial_rc {
            for (s, &rc) in state_a.iter_mut().zip(round_constants.iter()) {
                add_rc_and_sbox::<FP, 3>(s, rc);
            }
            for (s, &rc) in state_b.iter_mut().zip(round_constants.iter()) {
                add_rc_and_sbox::<FP, 3>(s, rc);
            }
            mds_fft(state_a, lambda16);
            mds_fft(state_b, lambda16);
        }

        // --- Last initial full round, interleaved ---
        {
            for (s, &rc) in state_a.iter_mut().zip(simd.packed_last_initial_rc.iter()) {
                add_rc_and_sbox::<FP, 3>(s, rc);
            }
            for (s, &rc) in state_b.iter_mut().zip(simd.packed_last_initial_rc.iter()) {
                add_rc_and_sbox::<FP, 3>(s, rc);
            }
            let input_a = *state_a;
            let input_b = *state_b;
            for (i, sa) in state_a.iter_mut().enumerate() {
                *sa = PackedKB::dot_product(&input_a, &simd.packed_fused_mi_mds[i]) + simd.packed_fused_bias[i];
            }
            for (i, sb) in state_b.iter_mut().enumerate() {
                *sb = PackedKB::dot_product(&input_b, &simd.packed_fused_mi_mds[i]) + simd.packed_fused_bias[i];
            }
        }

        // --- Partial rounds, interleaved ---
        {
            let mut split_a = InternalLayer16::from_packed_field_array(*state_a);
            let mut split_b = InternalLayer16::from_packed_field_array(*state_b);
            ...
            for r in 0..POSEIDON1_PARTIAL_ROUNDS {
                ...
                if r < POSEIDON1_PARTIAL_ROUNDS - 1 {
                    split_a.s0 += simd.packed_round_constants[r];
                    split_b.s0 += simd.packed_round_constants[r];
                }
                ...
                let first_row = &simd.packed_sparse_first_row[r];
                let first_row_hi: &[PackedKB; 15] = first_row[1..].try_into().unwrap();
                let partial_dot_a = PackedKB::dot_product(s_hi_a, first_row_hi);
                let partial_dot_b = PackedKB::dot_product(s_hi_b, first_row_hi);

                let s0_a = split_a.s0;
                let s0_b = split_b.s0;
                split_a.s0 = s0_a * first_row[0] + partial_dot_a;
                split_b.s0 = s0_b * first_row[0] + partial_dot_b;
                ...
                for j in 0..15 {
                    s_hi_a_mut[j] += s0_a * v[j];
                    s_hi_b_mut[j] += s0_b * v[j];
                }
            }

            *state_a = unsafe { split_a.to_packed_field_array() };
            *state_b = unsafe { split_b.to_packed_field_array() };
        }

        // --- Terminal full rounds, interleaved ---
        for round_constants in &simd.packed_terminal_rc {
            for (s, &rc) in state_a.iter_mut().zip(round_constants.iter()) {
                add_rc_and_sbox::<FP, 3>(s, rc);
            }
        ...
        for (s, init) in state_a.iter_mut().zip(initial_a) {
            *s += init;
        }
        for (s, init) in state_b.iter_mut().zip(initial_b) {
            *s += init;
        }
    }
}
