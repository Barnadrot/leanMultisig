...
/// Fused eq-table generation + round-0 sumcheck dot product.
///
/// Computes the dual eq polynomial (same as `compute_eval_eq_packed_dual`) into `out`,
/// AND simultaneously accumulates the sumcheck coefficients c0, c2 by multiplying
/// each generated weight entry by the corresponding `evals` entry.
///
/// This eliminates the second pass that `compute_product_sumcheck_polynomial` would
/// otherwise need over the weights and evals arrays.
///
/// Returns `DensePolynomial([c0, c1, c2])` where `c1 = sum - 2*c0 - c2`.
pub fn compute_eval_eq_packed_dual_with_dotproduct<EvF, EF>(
    eval_a: &[EF],
    eval_b: &[EF],
    out: &mut [EF::ExtensionPacking],
    scalar_a: EF,
...
where
    EvF: PrimeCharacteristicRing + Copy + Send + Sync,
    EF: ExtensionField<PF<EF>>,
    EF::ExtensionPacking: Algebra<EvF>,
{
    let packing_width = packing_width::<EF>();
    let log_packing_width = log2_strict_usize(packing_width);
    ...
    assert_eq!(eval_a.len(), eval_b.len());
    assert!(log_packing_width <= eval_a.len());
    let out_len = 1 << (eval_a.len() - log_packing_width);
    ...
    if eval_a.len() <= log_packing_width + 1 + LOG_NUM_THREADS {
        // Small case: generate weights using the non-fused path, then compute dot product.
        compute_eval_eq_packed_dual::<EF>(eval_a, eval_b, out, scalar_a, scalar_b);

        // Compute sumcheck polynomial from the generated weights.
        let (c0_packed, c2_packed) = evals[..half]
            .iter()
            .zip(evals[half..].iter())
            .zip(out[..half].iter().zip(out[half..].iter()))
        ...
        let c0: EF = unpack_efpacking_sum::<EF>(c0_packed);
        let c2: EF = unpack_efpacking_sum::<EF>(c2_packed);
        let c1 = sum - c0.double() - c2;
        return DensePolynomial::new(vec![c0, c1, c2]);
    }
    ...
    let eval_a_mid = &eval_a[LOG_NUM_THREADS..eval_len_min_packing];
    let eval_b_mid = &eval_b[LOG_NUM_THREADS..eval_len_min_packing];

    // The fill_buffer layout: buffer[0..N/2] corresponds to out[0..half] (lo),
    // and buffer[N/2..N] corresponds to out[half..] (hi).
    // We dispatch N/2 paired tasks, where task i processes both the lo chunk
    // (thread i) and the hi chunk (thread i + N/2), then computes the local
    // sumcheck contribution from paired (lo, hi) entries.
    let half_threads = NUM_THREADS_PADDED / 2;

    let (c0_packed, c2_packed) = (0..half_threads)
        .into_par_iter()
        .map(|i| {
            let lo_start = i * out_chunk_size;
            let hi_start = (i + half_threads) * out_chunk_size;

            // SAFETY: each task writes to non-overlapping regions of `out`.
            // The lo chunk is [lo_start..lo_start+chunk_size] and the hi chunk
            // is [hi_start..hi_start+chunk_size]. These don't overlap since
            // lo_start < half <= hi_start.
            let out_lo = unsafe {
                std::slice::from_raw_parts_mut(
                    out.as_ptr().add(lo_start) as *mut EF::ExtensionPacking,
                    out_chunk_size,
                )
            };
            let out_hi = unsafe {
                std::slice::from_raw_parts_mut(
                    out.as_ptr().add(hi_start) as *mut EF::ExtensionPacking,
                    out_chunk_size,
                )
            };

            // Generate eq weights for the lo chunk.
            eval_eq_with_packed_output_dual::<PF<EF>, EF>(
                eval_a_mid,
                eval_b_mid,
                out_lo,
            ...
            // Generate eq weights for the hi chunk.
            eval_eq_with_packed_output_dual::<PF<EF>, EF>(
                eval_a_mid,
                eval_b_mid,
                out_hi,
                parallel_buffer_a[i + half_threads],
                parallel_buffer_b[i + half_threads],
            );

            // Now compute sumcheck dot product for this paired chunk.
            // evals_lo is evals[lo_start..lo_start+chunk_size]
            // evals_hi is evals[hi_start..hi_start+chunk_size]
            let evals_lo = &evals[lo_start..lo_start + out_chunk_size];
            let evals_hi = &evals[hi_start..hi_start + out_chunk_size];

            let mut local_c0 = EF::ExtensionPacking::ZERO;
            let mut local_c2 = EF::ExtensionPacking::ZERO;

            for j in 0..out_chunk_size {
                let w_lo = out_lo[j];
                let w_hi = out_hi[j];
                let e_lo = evals_lo[j];
                let e_hi = evals_hi[j];
...
/// Unpack an `EFPacking<EF>` into its extension field elements and sum them.
/// This is a helper to avoid type inference issues when `EvF` is in scope.
#[inline]
fn unpack_efpacking_sum<EF>(packed: EF::ExtensionPacking) -> EF
where
    EF: ExtensionField<PF<EF>>,
{
    EFPacking::<EF>::to_ext_iter([packed]).sum()
}
...
mod tests {
    ...
    #[test]
    fn test_fused_eq_sumcheck_round0() {
        let packing_width = <F as Field>::Packing::WIDTH;
        let log_packing_width = log2_strict_usize(packing_width);
        let mut rng = StdRng::seed_from_u64(99);

        for n_vars in (log_packing_width + 1)..20 {
            let eval_a: Vec<EF> = (0..n_vars).map(|_| rng.random()).collect();
            let eval_b: Vec<EF> = (0..n_vars).map(|_| rng.random()).collect();
            let scalar_a: EF = rng.random();
            let scalar_b: EF = rng.random();
            ...
            // Generate random base-packed evals.
            let evals: Vec<<F as Field>::Packing> = (0..packed_len)
                .map(|_| <F as Field>::Packing::from_fn(|_| rng.random()))
                .collect();

            // Compute weights using the original function.
            let mut weights_ref = EFPacking::<EF>::zero_vec(packed_len);
            compute_eval_eq_packed_dual::<EF>(&eval_a, &eval_b, &mut weights_ref, scalar_a, scalar_b);

            // Compute the reference sumcheck polynomial coefficients.
            let half = packed_len / 2;
            let mut c0_ref = EFPacking::<EF>::ZERO;
            let mut c2_ref = EFPacking::<EF>::ZERO;
            for i in 0..half {
                c0_ref += weights_ref[i] * evals[i];
                c2_ref += (weights_ref[i + half] - weights_ref[i]) * (evals[i + half] - evals[i]);
            }
            let c0_ref_val: EF = super::unpack_efpacking_sum(c0_ref);
            let c2_ref_val: EF = super::unpack_efpacking_sum(c2_ref);

            // Compute the actual full sum for the polynomial.
            let actual_sum: EF = (0..packed_len)
                .map(|i| super::unpack_efpacking_sum::<EF>(weights_ref[i] * evals[i]))
                .sum();

            // Compute fused result.
            let mut weights_fused = EFPacking::<EF>::zero_vec(packed_len);
            let poly = super::compute_eval_eq_packed_dual_with_dotproduct(
                &eval_a,
                &eval_b,
                &mut weights_fused,
                scalar_a,
            ...
            assert_eq!(
                weights_fused, weights_ref,
                "Weights mismatch at n_vars={}",
                n_vars
            );

            // Verify sumcheck polynomial coefficients.
            assert_eq!(poly.coeffs[0], c0_ref_val, "c0 mismatch at n_vars={}", n_vars);
            assert_eq!(poly.coeffs[2], c2_ref_val, "c2 mismatch at n_vars={}", n_vars);
            let c1_ref = actual_sum - c0_ref_val.double() - c2_ref_val;
            assert_eq!(poly.coeffs[1], c1_ref, "c1 mismatch at n_vars={}", n_vars);
        }
    }
}