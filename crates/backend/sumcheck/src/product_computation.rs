...
pub fn run_product_sumcheck_factored<EF: ExtensionField<PF<EF>>>(
    ...
    let (c0_packed, c2_packed) = compute_factored_round_poly(evals, &factored, n_top, n_bot);
    ...
    let c1 = sum - c0.double() - c2;
    let first_poly = DensePolynomial::new(vec![c0, c1, c2]);

    prover_state.add_sumcheck_polynomial(&first_poly.coeffs, None);
    prover_state.pow_grinding(pow_bits);
    let r1: EF = prover_state.sample();
    sum = first_poly.evaluate(r1);
    ...
    let second_poly = compute_product_sumcheck_polynomial(
        &evals_folded, &weights_folded, sum,
    ...
    if n_rounds == 2 {
        let e2 = fold_ext_packed(&evals_folded, r2);
        let w2 = fold_ext_packed(&weights_folded, r2);
        return (MultilinearPoint(vec![r1, r2]), sum,
            MleOwned::ExtensionPacked(e2), MleOwned::ExtensionPacked(w2));
    }

    let (third_poly, folded_vecs) = fold_and_compute_product_sumcheck_polynomial(
        &evals_folded, &weights_folded, r2, sum,
        |e| EFPacking::<EF>::to_ext_iter([e]).collect(),
    );
    let folded = MleGroupOwned::ExtensionPacked(folded_vecs);
    ...
    let (mut challenges, folds, sum) = sumcheck_prove_many_rounds(
        folded, Some(r3), &ProductComputation {}, &vec![], None,
        prover_state, sum, None, n_rounds - 3, false, pow_bits,
    );
...
fn fold_base_packed<EF: ExtensionField<PF<EF>>>(evals: &[PFPacking<EF>], r: EF) -> Vec<EFPacking<EF>> {
...
fn fold_ext_packed<EF: ExtensionField<PF<EF>>>(data: &[EFPacking<EF>], r: EF) -> Vec<EFPacking<EF>> {
    let half = data.len() / 2;
    let r_packed = EFPacking::<EF>::from(r);
    let mut out: Vec<EFPacking<EF>> = unsafe { uninitialized_vec(half) };
    out.par_iter_mut().enumerate().for_each(|(i, o)| {
...
fn materialize_and_fold_factored<EF: ExtensionField<PF<EF>>>(
    factored: &FactoredWeights<EF>, n_top: usize, n_bot: usize, r: EF,
) -> Vec<EFPacking<EF>> {
    let half_top = n_top / 2;
    let r_packed = EFPacking::<EF>::from(r);
    let mut weights: Vec<EFPacking<EF>> = unsafe { uninitialized_vec(half_top * n_bot) };
    weights.par_chunks_mut(n_bot).enumerate().for_each(|(j, chunk)| {
        ...
        for (t, w) in chunk.iter_mut().enumerate() {
            let w_f = a_folded * factored.eq_bot_a[t] + b_folded * factored.eq_bot_b[t];
            let c_lo = factored.corrections[j * n_bot + t];
            let c_hi = factored.corrections[(j + half_top) * n_bot + t];
            *w = w_f + r_packed * (c_hi - c_lo) + c_lo;
        }
    });
    weights
}

fn compute_factored_round_poly<EF: ExtensionField<PF<EF>>>(
    evals: &[PFPacking<EF>], factored: &FactoredWeights<EF>, n_top: usize, n_bot: usize,
) -> (EFPacking<EF>, EFPacking<EF>) {
    let half_top = n_top / 2;
    assert_eq!(evals.len(), n_top * n_bot);
    const TILE: usize = 1024;
    let n_tiles = (n_bot + TILE - 1) / TILE;
    (0..n_tiles).into_par_iter().map(|tile_idx| {
        let tile_start = tile_idx * TILE;
        let tile_len = TILE.min(n_bot - tile_start);
        ...
        for j in 0..half_top {
            ...
            let lo_base = j * n_bot + tile_start;
            let hi_base = (j + half_top) * n_bot + tile_start;
            for t in 0..tile_len {
                let e_lo = evals[lo_base + t];
                let e_hi = evals[hi_base + t];
                acc_c0_a[t] += top_a_lo * e_lo;
                acc_c0_b[t] += top_b_lo * e_lo;
                let e_diff = e_hi - e_lo;
                acc_c2_a[t] += da_j * e_diff;
                acc_c2_b[t] += db_j * e_diff;
                acc_corr_c0[t] += factored.corrections[lo_base + t] * e_lo;
                acc_corr_c2[t] += (factored.corrections[hi_base + t]
                    - factored.corrections[lo_base + t]) * e_diff;
            }
        }
