...
use crate::{SplitEq, SumcheckComputation, sumcheck_prove_many_rounds};
...
pub struct FactoredEqTerm<EF: ExtensionField<PF<EF>>> {
    pub split_eq: SplitEq<EF>,
    pub scalar: EFPacking<EF>,
}

pub struct FactoredEqWeights<EF: ExtensionField<PF<EF>>> {
    pub terms: Vec<FactoredEqTerm<EF>>,
    pub sparse: Option<Vec<EFPacking<EF>>>,
}

impl<EF: ExtensionField<PF<EF>>> FactoredEqWeights<EF> {
    pub fn new(terms: Vec<(SplitEq<EF>, EF)>, sparse: Option<Vec<EFPacking<EF>>>) -> Self {
        Self {
            terms: terms
                .into_iter()
                .map(|(split_eq, scalar)| FactoredEqTerm {
                    split_eq,
                    scalar: EFPacking::<EF>::from(scalar),
                })
                .collect(),
            sparse,
        }
    ...
    pub fn get_packed(&self, i: usize) -> EFPacking<EF> {
        let mut val = match &self.sparse {
            Some(s) => s[i],
            None => EFPacking::<EF>::ZERO,
        };
        for term in &self.terms {
            val += term.split_eq.get_packed(i) * term.scalar;
        }
        val
    }
}
...
#[instrument(skip_all)]
pub fn compute_product_sumcheck_round0_factored_base<EF: ExtensionField<PF<EF>>>(
    evals: &[PFPacking<EF>],
    weights: &FactoredEqWeights<EF>,
    sum: EF,
) -> DensePolynomial<EF> {
    let n = evals.len();
    assert!(n.is_power_of_two());
    ...
    let (c0_packed, c2_packed) = (0..fold_size)
        .into_par_iter()
        .map(|i| {
            let e_lo = evals[i];
            let e_hi = evals[i + fold_size];
            let w_lo = weights.get_packed(i);
            let w_hi = weights.get_packed(i + fold_size);
            let c0 = w_lo * e_lo;
            let c2 = (w_hi - w_lo) * (e_hi - e_lo);
            (c0, c2)
        })
        .reduce(
...
#[instrument(skip_all)]
pub fn compute_product_sumcheck_round0_factored_ext<EF: ExtensionField<PF<EF>>>(
    evals: &[EFPacking<EF>],
    weights: &FactoredEqWeights<EF>,
    sum: EF,
) -> DensePolynomial<EF> {
...
#[instrument(skip_all)]
pub fn fold_and_compute_product_sumcheck_round1_factored<EF: ExtensionField<PF<EF>>>(
    evals: &[EFPacking<EF>],
    weights: &FactoredEqWeights<EF>,
    prev_folding_factor: EF,
    sum: EF,
) -> (DensePolynomial<EF>, Vec<EFPacking<EF>>, Vec<EFPacking<EF>>) {
    let n = evals.len();
    assert!(n.is_power_of_two());
    let prev_ff = EFPacking::<EF>::from(prev_folding_factor);
    let half = n / 2;
    ...
    let (c0_packed, c2_packed) = if half < PARALLEL_THRESHOLD {
        for i in 0..half {
            evals_folded[i] = evals[i] + prev_ff * (evals[i + half] - evals[i]);
            weights_folded[i] = weights.get_packed(i) + prev_ff * (weights.get_packed(i + half) - weights.get_packed(i));
        }
        evals_folded[..fold_size]
            .iter()
            .zip(evals_folded[fold_size..].iter())
            .zip(
                weights_folded[..fold_size]
                    .iter()
                    .zip(weights_folded[fold_size..].iter()),
            )
            .map(sumcheck_quadratic)
            .fold(
                (EFPacking::<EF>::ZERO, EFPacking::<EF>::ZERO),
        ...
        // Fold evals and materialize weights in parallel
        evals_folded
            .par_iter_mut()
            .zip(weights_folded.par_iter_mut())
            .enumerate()
            .for_each(|(i, (ef, wf))| {
                *ef = evals[i] + prev_ff * (evals[i + half] - evals[i]);
                *wf = weights.get_packed(i)
                    + prev_ff * (weights.get_packed(i + half) - weights.get_packed(i));
            });
        evals_folded[..fold_size]
            .par_iter()
