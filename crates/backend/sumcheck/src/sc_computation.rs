...
{
    let n_multi = multilinears.len();

    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        let mut point_buf = Vec::with_capacity(n_multi);
        let mut diffs = Vec::with_capacity(n_multi);

        // z = 0: fill point_buf with lo values
        ...
        // Compute diffs and set running to hi (= lo + diff = value at z=1)
        for (j, m) in multilinears.iter().enumerate() {
            let hi = m[i + fold_size];
            diffs.push(hi - point_buf[j]);
            point_buf[j] = hi;
        }
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let folded_f: Vec<Vec<FT>> = (0..n_multi)
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        // z = 0: fold and fill point_buf with lo values
        for (j, m) in multilinears.iter().enumerate() {
            let lo = fold_f(m, i);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut point_buf = vec![EFPacking::<EF>::ZERO; n_multi];
            let mut diffs = vec![EFPacking::<EF>::ZERO; n_multi];

            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];

                // z = 0: fill point_buf with lo values, compute diffs
                for (j, m) in multilinears.iter().enumerate() {
                    let lo = m[i];
                    let hi = m[i + fold_size];
                    point_buf[j] = lo;
    ...
    SC: SumcheckComputation<EF>,
{
    let n_multi = multilinears.len();
    let prev_folded_size = 2 * compute_fold_size;
    let folded_f: Vec<Vec<EFPacking<EF>>> = (0..n_multi)
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            for k in 0..packed_hi {
                ...
                // z = 0: fold and fill point_buf with lo values, compute diffs
                for (j, m) in multilinears.iter().enumerate() {
                    let lo = fold_f(m, i);
                    let hi = fold_f(m, i + compute_fold_size);
                    unsafe {
