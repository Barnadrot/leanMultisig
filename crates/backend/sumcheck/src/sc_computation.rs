    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        ...
        // z = 0
        let mut point: Vec<IF> = rows.iter().map(|r| r[0]).collect();
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        // z = 0
        let mut point: Vec<FT> = rows_f.iter().map(|r| r[0]).collect();
        let mut eval_0 = eval_fn(computation, &point, extra_data);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut rows = vec![[EFPacking::<EF>::ZERO, EFPacking::<EF>::ZERO, EFPacking::<EF>::ZERO]; n_cols];
            let mut point = EFPacking::<EF>::zero_vec(n_cols);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let base = b_lo << log_packed_hi;
            let mut block_acc = zero();
            let mut rows_f = vec![[EFPacking::<EF>::ZERO, EFPacking::<EF>::ZERO, EFPacking::<EF>::ZERO]; n_cols];
            let mut point = EFPacking::<EF>::zero_vec(n_cols);
            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];
