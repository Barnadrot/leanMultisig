...
{
    let n_cols = multilinears.len();
    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        let mut lo_buf: Vec<IF> = Vec::with_capacity(n_cols);
        let mut diff_buf: Vec<IF> = Vec::with_capacity(n_cols);
        let mut running_buf: Vec<IF> = Vec::with_capacity(n_cols);
        for m in multilinears {
            let lo = m[i];
        ...
        // z = 0
        let mut eval_0 = eval_fn(computation, &lo_buf, extra_data);
        ...
        for _ in 1..degree {
            for j in 0..n_cols {
                running_buf[j] += diff_buf[j];
            }
            let mut eval = eval_fn(computation, &running_buf, extra_data);
            if let Some(eq) = eq_val {
                eval *= eq;
            }
            evals.push(eval);
...
where
    EF: ExtensionField<PF<EF>>,
    IF: Copy + Send + Sync,
    FT: PrimeCharacteristicRing + Copy + Sub<Output = FT> + Add<Output = FT> + AddAssign + Send + Sync,
    SC: SumcheckComputation<EF>,
{
    let prev_folded_size = 2 * compute_fold_size;
    let n_cols = multilinears.len();

    let folded_f: Vec<Vec<FT>> = (0..n_cols)
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        let mut lo_buf: Vec<FT> = Vec::with_capacity(n_cols);
        let mut diff_buf: Vec<FT> = Vec::with_capacity(n_cols);
        let mut running_buf: Vec<FT> = Vec::with_capacity(n_cols);
        for (j, m) in multilinears.iter().enumerate() {
            let lo = fold_f(m, i);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut lo_buf = Vec::with_capacity(n_cols);
            let mut diff_buf = Vec::with_capacity(n_cols);
            let mut running_buf = Vec::with_capacity(n_cols);
            for k in 0..packed_hi {
                ...
                // z = 0
                let mut e0 = eval_fn(computation, &lo_buf, extra_data);
                e0 *= eq_val;
                block_acc[0] += e0;
                ...
                for d in 1..degree {
                    ...
                    let mut ev = eval_fn(computation, &running_buf, extra_data);
                    ev *= eq_val;
                    block_acc[d] += ev;
                }
            }
    ...
    SC: SumcheckComputation<EF>,
{
    let prev_folded_size = 2 * compute_fold_size;
    let n_cols = multilinears.len();
    let folded_f: Vec<Vec<EFPacking<EF>>> = (0..n_cols)
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let base = b_lo << log_packed_hi;
            let mut block_acc = zero();
            let mut lo_buf: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            let mut diff_buf: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            let mut running_buf: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];
