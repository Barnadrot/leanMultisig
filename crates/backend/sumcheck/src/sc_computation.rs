    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        ...
        // z = 0
        let mut point: Vec<IF> = Vec::with_capacity(n_cols);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        // z = 0
        let mut point: Vec<FT> = Vec::with_capacity(n_cols);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut rows: Vec<[EFPacking<EF>; 3]> = Vec::with_capacity(n_cols);
            let mut point: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let base = b_lo << log_packed_hi;
            let mut block_acc = zero();
            let mut rows_f: Vec<[EFPacking<EF>; 3]> = Vec::with_capacity(n_cols);
            let mut point: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];
