    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        ...
        let mut point_buf = Vec::with_capacity(n_polys);

        // z = 0
        point_buf.extend(rows.iter().map(|row| row[0]));
        let mut eval_0 = eval_fn(computation, &point_buf, extra_data);
        ...
        for _ in 1..degree {
            ...
            point_buf.clear();
            point_buf.extend(rows.iter().map(|row| row[2]));
            let mut eval = eval_fn(computation, &point_buf, extra_data);
            if let Some(eq) = eq_val {
                eval *= eq;
            }
            evals.push(eval);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let folded_f: Vec<Vec<FT>> = (0..n_polys)
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        // z = 0
        point_buf.extend(rows_f.iter().map(|row| row[0]));
        let mut eval_0 = eval_fn(computation, &point_buf, extra_data);
        ...
        for _ in 1..degree {
            ...
            point_buf.clear();
            point_buf.extend(rows_f.iter().map(|row| row[2]));
            let mut eval = eval_fn(computation, &point_buf, extra_data);
            if let Some(eq) = eq_mle_eval {
                eval *= eq;
            }
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut rows = vec![[EFPacking::<EF>::ZERO; 3]; n_polys];
            let mut point_buf = EFPacking::<EF>::zero_vec(n_polys);
            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];

                for (r, m) in rows.iter_mut().zip(multilinears.iter()) {
                    let lo = m[i];
                    let hi = m[i + fold_size];
                    *r = [lo, hi - lo, hi];
                }
                ...
                // z = 0
                for (dst, r) in point_buf.iter_mut().zip(rows.iter()) {
                    *dst = r[0];
                }
                let mut e0 = eval_fn(computation, &point_buf, extra_data);
                e0 *= eq_val;
                block_acc[0] += e0;
                ...
                for d in 1..degree {
                    ...
                    let mut ev = eval_fn(computation, &point_buf, extra_data);
                    ev *= eq_val;
                    block_acc[d] += ev;
                }
            }
    ...
    SC: SumcheckComputation<EF>,
{
    let prev_folded_size = 2 * compute_fold_size;
    let n_polys = multilinears.len();
    let folded_f: Vec<Vec<EFPacking<EF>>> = (0..n_polys)
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let base = b_lo << log_packed_hi;
            let mut block_acc = zero();
            let mut rows_f = vec![[EFPacking::<EF>::ZERO; 3]; n_polys];
            let mut point_buf = EFPacking::<EF>::zero_vec(n_polys);
            for k in 0..packed_hi {
                let i = base + k;
                let eq_val = eq_hi[k];

                for (j, (r, m)) in rows_f.iter_mut().zip(multilinears.iter()).enumerate() {
                    let lo = fold_f(m, i);
                    let hi = fold_f(m, i + compute_fold_size);
                    unsafe {
                ...
                for (dst, r) in point_buf.iter_mut().zip(rows_f.iter()) {
                    *dst = r[0];
                }
                let mut e0 = eval_fn(computation, &point_buf, extra_data);
                e0 *= eq_val;
