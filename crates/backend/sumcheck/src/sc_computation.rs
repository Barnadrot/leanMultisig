    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_at = |i: usize, eq_val: Option<EFT>| -> Vec<EFT> {
        // Use column-major layout: separate lo/diff/running buffers.
        // `lo` serves as the point buffer for z=0, `running` is reused for z=2,3,...
        let mut lo: Vec<IF> = Vec::with_capacity(n_cols);
        let mut diff: Vec<IF> = Vec::with_capacity(n_cols);
        let mut running: Vec<IF> = Vec::with_capacity(n_cols);
        ...
        // z = 0
        let mut eval_0 = eval_fn(computation, &lo, extra_data);
        ...
        // z = 2, 3, ...
        for _ in 1..degree {
            for (r, d) in running.iter_mut().zip(diff.iter()) {
                *r += *d;
            }
            let mut eval = eval_fn(computation, &running, extra_data);
            if let Some(eq) = eq_val {
                eval *= eq;
            }
            evals.push(eval);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let compute_iteration = |i: usize| -> Vec<FT> {
        ...
        let mut lo: Vec<FT> = Vec::with_capacity(n_cols);
        let mut diff: Vec<FT> = Vec::with_capacity(n_cols);
        let mut running: Vec<FT> = Vec::with_capacity(n_cols);

        for (j, m) in multilinears.iter().enumerate() {
            let l = fold_f(m, i);
            let h = fold_f(m, i + compute_fold_size);
            unsafe {
                let ptr = folded_f[j].as_ptr() as *mut FT;
                *ptr.add(i) = l;
                *ptr.add(i + compute_fold_size) = h;
            }
            lo.push(l);
            diff.push(h - l);
            running.push(h);
    ...
    SC: SumcheckComputation<EF>,
{
    ...
    let sums: Vec<EFPacking<EF>> = (0..n_lo)
        ...
        .map(|b_lo| {
            ...
            let mut lo: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            let mut diff: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            let mut running: Vec<EFPacking<EF>> = Vec::with_capacity(n_cols);
            for k in 0..packed_hi {
                ...
                // z = 0
                let mut e0 = eval_fn(computation, &lo, extra_data);
                e0 *= eq_val;
                block_acc[0] += e0;

                // z = 2, 3, ...
                for d in 1..degree {
                    for (r, dd) in running.iter_mut().zip(diff.iter()) {
                        *r += *dd;
                    }
                    let mut ev = eval_fn(computation, &running, extra_data);
                    ev *= eq_val;
                    block_acc[d] += ev;
                }
            }
